...
std::vector<std::shared_ptr<sphere>> random_scene() {
	...
	spheres.push_back(std::make_shared<sphere>(point3(0, 1, 0), 1.0, material1));
	spheres.push_back(std::make_shared<sphere>(point3(-4, 1, 0), 1.0, material2));
	spheres.push_back(std::make_shared<sphere>(point3(4, 1, 0), 1.0, material3));
...
color ray_color(const ray& r, const bvh_node &tree, int depth) {
	...
	if (hit_anything)
	{
		...
		{
			return attenuation * ray_color(scattered, tree, depth - 1);
		}
		return color(0, 0, 0);
	}
	...
	double3 unit_direction = linear_algebra::normalize(r.direction());
...
renderer::renderer() :
	...
	bvh_tree = bvh_node(scene, size_t(0), scene.size(), 0, 1);
};
		...
		for (double i = 0; i < image_width; ++i) {
			...
			for (double sj = 0; sj < sqrt_samples_per_pixel; ++sj) {
				for (double si = 0; si < sqrt_samples_per_pixel; ++si) {
					...
					pixel_color += ray_color(r, bvh_tree, max_bounce_depth);
				}
			}
			//gama correction
